...
class DraftModelSpeculator(BaseSpeculator):
    ...
    def sample_draft(
        ...
        if draft_logits is not None:
            # Probabilistic (gumbel) sampling: compute full logits for sampling
            # and optionally for adaptive-verify prob collection.
            logits = self.model.compute_logits(hidden_states)
            ...
            if self.needs_draft_probs:
                # Capture the accept-prob proxy of the sampled draft token for
                # the adaptive verifier. Done alongside — not instead of — normal
                # sampling so the draft distribution is unchanged.
        ...
        elif self.needs_draft_probs:
            # Greedy sampling + prob collection: must compute full logits to
            # extract per-token probabilities. (_greedy_sample_draft may skip
            # full logits when use_local_argmax_reduction is set, which is why
            ...
            # Greedy sampling, no prob collection needed: use _greedy_sample_draft
            # which supports use_local_argmax_reduction (TP-local top-k reduction
            # instead of full logit all-gather).
            draft_tokens = self._greedy_sample_draft(hidden_states)
        return draft_tokens
